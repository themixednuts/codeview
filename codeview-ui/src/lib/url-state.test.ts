...
describe("url-state", () => {
  ...
  it("sorts, dedupes, and caps expanded ids", () => {
    const ids = Array.from(
      { length: EXPLORER_EX_LIMIT + 8 },
      (_, i) => `std::m${String(EXPLORER_EX_LIMIT + 8 - i).padStart(2, "0")}`,
    );
  ...
  it("deletes default and empty explorer params on serialize", () => {
    const next = serializeExplorerState(
      url(
        "?view=graph&layout=split&q=old&k=Trait&ex=a&gbi=1&viz=grid&td=a&sd=b&src=c&peek=d&rel=e",
      ),
      {
        view: "docs",
        layout: null,
