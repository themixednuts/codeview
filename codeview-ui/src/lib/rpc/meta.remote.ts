 ...
 */
async function loadCrateMeta({ name, version }: { name: string; version?: string }) {
	...
	return perf.timeAsync(
		...
		async () => {
			...
			return result satisfies CrateMeta | null;
		},
		{
...
export const getCrateMeta = query(CrateRefSchema, loadCrateMeta);

export const getStaticCrateMeta = prerender(CrateRefSchema, loadCrateMeta, {
	dynamic: true,
});
