<script lang="ts" module>
	import { cn, refAttachment } from '$lib/shadcn/utils.js';
...
<script lang="ts">
	...
	const attachAnchorRef = refAttachment<HTMLAnchorElement>((node) => (ref = node));
	const attachButtonRef = refAttachment<HTMLButtonElement>((node) => (ref = node));
</script>

{#if href}
	<a
		{@attach attachAnchorRef}
		data-slot="button"
