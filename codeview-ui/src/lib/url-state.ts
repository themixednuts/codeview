...
const DOC_LAYOUT_VALUES = [
  "classic",
  "reading",
  "split",
] as const satisfies readonly ExplorerDocLayout[];
const VIZ_VALUES = [
  "graph",
  "treemap",
  "sunburst",
  "grid",
] as const satisfies readonly ExplorerVizMode[];
...
function readExpandedIds(params: AppSearchParams): string[] {
...
export function serializeExplorerState(base: AppUrl, patch: Partial<ExplorerViewState>): URL {
 ...
 */
export function kindFilterHref(base: AppUrl, kind: NodeKind, currentlyActive: boolean): string {
  const url = new URL(base.href);
...
export function parseHomeState(url: AppUrl, options: HomeStateOptions = {}): HomeViewState {
  const { searchParams } = url;
