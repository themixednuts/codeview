...
export function mergeTraitMemberDocumentation(
  ...
  for (const implNode of related) {
    if (implNode.kind !== "Impl" || implNode.impl_category !== "Trait" || !implNode.impl_trait) {
      continue;
    }
    const traitView = traitViews.get(implNode.impl_trait);
    if (!traitView) continue;

    const traitRelated = new Map(traitView.detail.relatedNodes.map((node) => [node.id, node]));
    const traitMembers = new Map<string, Node>();
    for (const edge of traitView.detail.edges) {
      if (edge.from !== traitView.detail.node.id || !["Defines", "Contains"].includes(edge.kind)) {
        continue;
      }
