...
function renderPolyTrait(p: PolyTrait): string {
  const hrtb =
    p.hrtb_params && p.hrtb_params.length > 0
...
function renderFunctionPointer(sig: FunctionPointerSig): string {
  const hrtb =
    sig.hrtb_params && sig.hrtb_params.length > 0
...
export function renderGenericParam(param: GenericParam): string {
  ...
  switch (kind.kind) {
    case "Lifetime": {
      const outlives =
        kind.outlives && kind.outlives.length > 0 ? `: ${kind.outlives.join(" + ")}` : "";
      return `${param.name}${outlives}`;
    }
    case "Type": {
      const bounds =
        kind.bounds && kind.bounds.length > 0
...
export function renderWherePredicate(pred: WherePredicate): string {
  switch (pred.kind) {
    case "Bound": {
      const hrtb =
        pred.hrtb_params && pred.hrtb_params.length > 0
