<script lang="ts">
	...
	const defaultSize = $derived(
		name === 'github' || name === 'search'
			? 16
			: name.startsWith('chevron') || name === 'arrow-right'
				? 14
				: 12,
	);
	const resolvedSize = $derived(size ?? defaultSize);
	const isGithub = $derived(name === 'github');
</script>
