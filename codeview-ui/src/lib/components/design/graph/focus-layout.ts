import type { Edge, EdgeKind, Node } from "#lib/schema.js";
import type { DesignNode, DesignRelation } from "#lib/design/live-node.js";
...
export function layoutFocusGraph(model: FocusGraphModel, size: FocusGraphSize): FocusGraphLayout {
...
function layoutSide(
  groups: FocusGraphGroup[],
  centerY: number,
  row: number,
  gap: number,
): PositionedGroup[] {
  const total = groups.reduce(
    (sum, group, index) => sum + group.items.length * row + (index ? gap : 0),
    0,
  );
...
function sumEdges(groups: FocusGraphGroup[]): number {
  return groups.reduce(
    (sum, group) => sum + group.items.reduce((inner, item) => inner + item.edges.length, 0),
    0,
  );
}
