...
describe('buildBaseScene', () => {
	...
	it('filters edges according to opts', () => {
		...
		const base = buildBaseScene(graph, center, 'ego', {
...
describe('buildScene', () => {
	...
	it('supports projected graphs with trait filtering metadata policy', () => {
		const trait = makeNode('my_crate::Display', 'Display', 'Trait');
		const graph = makeGraph(
			[center, dep, trait],
			[
				makeEdge('my_crate::Foo', 'my_crate::Bar', 'UsesType'),
				makeEdge('my_crate::Foo', 'my_crate::Display', 'Implements'),
			],
		);

		const projected = projectGraphForRendering(graph, center, {
			showStructural: true,
			showSemantic: true,
			layoutMode: 'ego',
		});

		expect(projected.graph.nodes.some((n) => n.kind === 'Trait')).toBe(false);

		const scene = buildScene(projected.graph, center, 'ego', {
			showStructural: true,
			showSemantic: true,
		});

		expect(scene.nodes.some((n) => n.node.kind === 'Trait')).toBe(false);
		expect(scene.edges.length).toBe(scene.labels.length);
	});

	it('supports projected overflow summary nodes', () => {
		const denseNeighbors = Array.from({ length: 220 }, (_, i) =>
			makeNode(`my_crate::Dep${i}`, `Dep${i}`, 'Function'),
		);
		const graph = makeGraph(
			[center, ...denseNeighbors],
			denseNeighbors.map((node) => makeEdge(center.id, node.id, 'UsesType')),
		);
		...
		expect(projected.graph.nodes.some((n) => isSyntheticProjectionNodeId(n.id))).toBe(true);
		...
		expect(scene.nodes.length).toBeLessThanOrEqual(24);
		expect(scene.edges.length).toBe(scene.labels.length);
	});
