...
test.describe("State Corruption & Race Conditions", () => {
  ...
  test("returning to crate root from detail view restores tree state", async ({
    page,
    safeGoto,
  }) => {
    const sidebar = await setupCrateView(page, safeGoto);
  ...
  test("SSE connections are cleaned up during rapid crate switching", async ({
    page,
    safeGoto,
  }) => {
    await safeGoto("/");
  ...
  test("opening multiple browser tabs does not cause SSE conflicts", async ({
    page,
    context,
    safeGoto,
  }) => {
  ...
  test("kind filter combined with search does not cause empty state", async ({
    page,
    safeGoto,
  }) => {
    const sidebar = await setupCrateView(page, safeGoto);

    await expect(sidebar.locator(".flex-wrap button[data-kind]").first()).toBeVisible({
      timeout: 5_000,
    });
    await page.evaluate(() => {
