...
async function clickNavigatingGraphLink(
  page: import("@playwright/test").Page,
): Promise<string | null> {
  ...
  for (let i = 0; i < Math.min(count, 24); i += 1) {
    ...
    await link.click();
    await expect
      .poll(() => toPathname(page.url(), page.url()), { timeout: 10_000 })
      .toBe(targetPath);
    return href;
  }
...
test.describe("State Sync", () => {
  ...
  test("URL navigation highlights tree item", async ({ page, safeGoto }) => {
    ...
    const selectedItem = sidebarAfter.locator(
      '.overflow-auto a.ring-1, .overflow-auto a[class*="ring-1"]',
    );
  ...
  test("breadcrumbs match node ancestry", async ({ page, safeGoto }) => {
    ...
    const breadcrumbs = page.locator(
      'nav[aria-label="Breadcrumb"] a, nav[aria-label*="readcrumb"] a',
    );
  ...
  test("structural toggle syncs with URL", async ({ page, safeGoto }) => {
    ...
    const structuralBtn = page.locator(
      'button:has-text("Structural"), button[data-edge-toggle="structural"]',
    );
